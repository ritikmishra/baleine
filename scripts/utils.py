...
def world_has_trait(scninfo: List[ScenarioInfoElement], world: World, target_trait_id: int,
                    include_world_characteristics=True) -> bool:
    """
    ...
    if include_world_characteristics:
        ...
        characteristic_inherits_from_target = any(trait_inherits_from_trait(scninfo, characteristic, target_trait_id)
                                                  for characteristic in characteristic_traits)

        if target_trait_id in characteristic_traits or characteristic_inherits_from_target:
            return True
...
def trait_inherits_from_trait(scninfo: List[ScenarioInfoElement], child_trait: Union[Trait, int],
    ...
    try:
        ...
        return parent_trait in childs_parent_list or any(
            trait_inherits_from_trait(scninfo, trait, parent_trait) for trait in childs_parent_list)
...
def trait_under_construction(squashed_trait_dict: Dict[int, Union[int, Trait]], trait_id: int) -> bool:
    ...
    if isinstance(squashed_trait_dict[trait_id], int):
        return False  # its a simple structure, world has it, and its built
    if squashed_trait_dict[trait_id].build_complete is not None:
        return True

    return False


def world_has_fully_built_trait(scninfo: List[ScenarioInfoElement], world: World, target_trait_id: int):
    return (world_has_trait(scninfo, world, target_trait_id, True)
